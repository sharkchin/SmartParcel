...
{
    class ParcelDirFacts : IGeometryAlgorithm
    {
        ...
        public object ProcessRecord(AbstractRecord record, bool isSubByTask)
        {
            ...
            facts.Add("DirGeometry", SqlGeometry.Null);
            facts.Add("ParcelDir", "NA");
            facts.Add("ClosestStName", "NA");
            ...
            if (impactors.Count > 1)
            {
                ...
                {
                    AbstractRecord impactor = impactors.ElementAt(i);
                    string name = impactor["NAME"] as string;
            ...
            else
            {
                ...
                closestrdname = impactor["NAME"] as string;
            }
            if (closestrd == SqlGeometry.Null)
                return facts;
            facts["ClosestStName"]= closestrdname;
            ...
            if (dirvec != new Vector(0, 0))
            {
                facts["DirGeometry"] = GeoUtils.Points2LineString(new List<Vector> { pcvec, pcvec + dirvec }, SRID);
                facts["ParcelDir"] = Angle2Dir(Vector.AngleBetween(new Vector(1, 0), dirvec));
            }
            return facts;
