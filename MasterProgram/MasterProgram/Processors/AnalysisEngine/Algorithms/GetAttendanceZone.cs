...
{
    class GetAttendanceZone : IGeometryAlgorithm
    {
        ...
        public object ProcessRecord(AbstractRecord record, bool isSubByTask)
        {
            ...
            //DataRecord facts=new DataRecord(new string[] {});
            facts.Add("ElementarySchoolId", null);
            facts.Add("MiddleSchoolId", null);
            facts.Add("HighSchoolId", null);
            facts.Add("SchoolDistrictId", null);
            facts.Add("ElementarySchoolName", null);
            facts.Add("MiddleSchoolName", null);
            facts.Add("HighSchoolName", null);
            facts.Add("SchoolDistrictName", null);
            facts.Add("ElementarySchoolAccuracy", -1);
            facts.Add("MiddleSchoolAccuracy", -1);
            facts.Add("HighSchoolAccuracy", -1);
            ...
            try
            {
                if (impactors.Count > 0)
                {
                    for (int i = 0; i < impactors.Count; i++)
                    {
                        ...
                        {
                            string schtype = impactor["ED_LEVEL"] as string;
                            switch (schtype)
                            {
                                case "P":
                                    found++;
                                    facts["ElementarySchoolId"] = impactor["NCES_SCHID"];
                                    facts["SchoolDistrictId"] = impactor["NCES_DISID"];
                                    facts["ElementarySchoolAccuracy"] = 2;
                                    break;
                                case "M":
                                    found++;
                                    facts["MiddleSchoolId"] = impactor["NCES_SCHID"];
                                    facts["SchoolDistrictId"] = impactor["NCES_DISID"];
                                    facts["MiddleSchoolAccuracy"] = 2;
                                    break;
                                case "H":
                                    found++;
                                    facts["HighSchoolId"] = impactor["NCES_SCHID"];
                                    facts["DistrictId"] = impactor["NCES_DISID"];
                                    facts["HighSchoolAccuracy"] = 2;
                                    break;
                            }
                            if (found == 3)
                                return facts;
