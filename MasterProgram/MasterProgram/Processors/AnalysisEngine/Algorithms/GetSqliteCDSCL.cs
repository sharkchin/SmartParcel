...
{
    class GetSqliteCDSCL : IGeometryAlgorithm
    {
        ...
        public object ProcessRecord(AbstractRecord record, bool isSubByTask)
        {
            ...
            if (reclist.Count == 0)
                return new Dictionary<string,object>{{"CDS",false},{"CL",false}};
            if (reclist.Count == 1)
                return new Dictionary<string,object>{{"CDS",reclist[0]["CDS"]??false},{"CL",reclist[0]["CL"]??false}};
            ...
            for (int i = 0; i < reclist.Count; i++)
            {
                ...
                {
                    Console.WriteLine("exception in GetSqliteCDSCL:"+e.StackTrace);
                }
            }
            return new Dictionary<string, object> { { "CDS", bestRec["CDS"]??false }, { "CL", bestRec["CL"]??false } };
        }
